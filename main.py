...
def get_program_detail(program, rCount, csvfile, id_csvfile):  
    ...
    for i in range(reviewPages):
        req = requests.get('https://api.frientrip.com/Reviews/getItemsByProductId?filter=%7B%22productId%22:%22'+str(program['id'])+'%22,%22currentPage%22:'+str(i+1)+'%7D')
        datas = req.text
...
if __name__=='__main__':
    ...
    while 1:
        ...
        req = requests.get('https://api.frientrip.com/Products/getItems?filter=%7B%22tagId%22:139,%22type%22:%22Ticket%22,%22currentPage%22:'+str(num)+',%22perPage%22:'+str(item_num)+'%7D&version=v2')
        datas = req.text
        if datas=='[]':
            print('END!')
            break
